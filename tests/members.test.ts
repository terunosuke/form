...
describe("梁の拾い出し(梁底の独立管理 E-25)", () => {
  ...
  it("壁の上に梁が乗ると、梁底は壁部分を除いた区間に分割される", () => {
    const r = takeoffBeam(
      { ...beam, bottomDeductionsU: [[1000, 2000]] },
      config(),
    );
    const bottoms = r.faces.filter((f) => f.faceType === "beam_bottom");
    expect(bottoms).toHaveLength(2); // 0〜1000 と 2000〜5400
    expect(bottoms[0]!.width).toBe(1000);
    expect(bottoms[0]!.originUOffset).toBe(0);
    expect(bottoms[1]!.width).toBe(3400);
    expect(bottoms[1]!.originUOffset).toBe(2000);
    // 桟木・鋼管も区間ごとに割り付く(壁区間には存在しない)
    const totalArea = bottoms.reduce((s, f) => s + f.netArea, 0);
    expect(totalArea).toBe((1000 + 3400) * 744);
    expect(bottoms[0]!.batten.countTotal.rounded).toBeGreaterThan(0);
    expect(bottoms[0]!.notes.some((n) => n.includes("壁部分には梁底を設けない"))).toBe(true);
  });

  it("梁底の全長が壁に乗る場合は梁底面を生成しない", () => {
    const r = takeoffBeam({ ...beam, bottomDeductionsU: [[0, 5400]] }, config());
    expect(r.faces.filter((f) => f.faceType === "beam_bottom")).toHaveLength(0);
  });
});
